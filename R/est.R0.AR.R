...
#' 
#' In the simple SIR model, the relation between R0 and the Attack Rate is in
#' the form \eqn{R0 = -ln((1-AR)/S0) / (\code{AR} - (1-S0))}.
#' 
#' If the population size is provided, the variance of R0 is estimated using
#' the delta method.  The hypothesis are that of homogeneous mixing, no more
#' transmission (epidemic ended), no change in transmission or interventions
#' during the epidemic. This estimate may be correct in closed populations, and
#' may be less valid in other cases.
#' 
#' The correction for incomplete susceptibility is based on the SIR model
#' equations.
#' 
#' CI is computed for the attack rate considering the population size
#' (\eqn{CI(\code{AR}) = \code{AR} +/- 1.96*sqrt(AR*(1-AR)/n)}), and so the CI
#' for the reproduction number is computed with this extreme values.
#' 
#' @param AR Attack rate as a percentage from total population
#' @param incid Sum of incident cases, possibly in the form of a vector of
#' counts.
...
#' @return A list with components: \item{epid}{The vector of incidence, after
#' being correctly formated by \code{\link{check.incid}}. Used only by
#' plot.fit.} \item{R}{The estimate of the reproduction ratio.}
#' \item{conf.int}{The 95\% confidence interval for the R estimate.}
#' \item{AR}{Original attack rate.} \item{begin.nb}{First date of incidence
#' record. Used only by plot.fit.} \item{end.nb}{Last date of incidence record.
#' Used only by plot.fit.} \item{method}{Method used for the estimation.}
...
#' @references Dietz, K. "The Estimation of the Basic Reproduction Number for
#' Infectious Diseases." Statistical Methods in Medical Research 2, no. 1
#' (March 1, 1993): 23-41.
#' @examples
#' #Loading package
#' library(R0)
