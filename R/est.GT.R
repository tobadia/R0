...
#' Find the best-fitting GT distribution for a series of serial interval
#' 
#' Generation Time distribution can be estimated by two inputs methods. User
#' can either provide two vectors of dates or a unique vector of reported
#' serial intervals. If two vectors are provided, both onset.dates vectors
#' should be of same length. Element i is the onset date for individual i. This
#' means that infector k (symptoms on day infector.onset.dates[k]) infected
#' infectee k (symptoms on day infectee.onset.dates[k]) If only
#' \code{serial.interval} is provided, each record is assumed to be the time
#' elapsed between each pair of infector and infectee.
#' 
#' When \code{request.plot} is set to TRUE, a graphical output provides
#' standardized histogram of observed data along with the best-fitting adjusted
#' model.
#' 
...
#' @param request.plot Should data adjsument be displayed at the end?
#' @param \dots Parameters passed to other functions (useful for hidden
#' parameters of \code{\link{generation.time}})
#' @return A \code{R0.GT} object that complies with
#' \code{\link{generation.time}} distribution requirements of the R0 package
