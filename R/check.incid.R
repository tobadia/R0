...
#' 
#' check.incid handles everything related to incidence content integrity. It is
#' designed to generate an output which comply with estimation functions
#' requirements. Epidemic data can be provided as an epitools object (see
#' below) or as vectors (incidence, dates, or both). When dates are provided,
#' they can be in a separate \code{t} vector, or computed with the first value
#' and a time step. In the end, the function returns a list with "epid" and "t"
#' values. If you plan on using estimation functions on their own (and not
#' throught est.R0), be aware that any incorrect input format will result in
#' erratic behavior and/or crash.
#' 
#' Object \code{incid} is either a list or data.frame. Expect item/column
#' "$dates"" and/or "$stratum3". This is expected to work with objects created
#' by epitools package (tested with v0.5-6).
#' 
#' Epicurve.dates returns (among other things) a list with $dates object. This
#' list gives incidence per day. Other epicurve methods return $dates along
#' with a $<time_period> object and a $stratum3, which contains respectively
#' daily incidence data agregated by the given time period, and the same data
#' with colnames that comply with R standard time notation.
#' 
#' E.g.: epicurve.weeks returns $dates, $weeks and $stratum3. $stratum3 object
#' is a list of dates (correct syntax), where each date is repeated to reflect
...
#' @return A list with components \code{incid} and \code{t}.
